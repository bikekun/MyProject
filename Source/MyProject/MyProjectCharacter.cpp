...
void AMyProjectCharacter::CheckTestParam()
{
	if (iTest == 0)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, FString::Printf(TEXT("test value: %d"),iTest));
	}
	else if (iTest == 1)
	{
		
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("test value: %d"), iTest));
	}

	else if (iTest > 1)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Black, FString::Printf(TEXT("test value: %d"), iTest));
	}
}
